...
// Lorsque l'internaute clique sur le lien 'deconnexion', il transmet dans le même temps dans l'URL les paramètres 'action=deconnexion'
//La conditiob IF permet de vérifier si l'indice 'action' est bien définit dans l'URL et qu'il a pour valeur 'deconnexion', on entre dans le IF seulement dans le cas où l'internaute clique sur 'deconnexion'
if(isset($_GET['action']) && $_GET['action'] == 'deconnexion')
{
    //Pour que l'internaute soit déconnecté, il faut supprimer la session ou vider une partie afin que l'indice 'user' dans la session ne soit plus définit
...
if($_POST)
{
    // On selectionne tout en BDD à condition que le champ pseudo ou email soit égal à la donnée saisie par l'internaute dans lee formulaire dans le champ pseudo/email
    ...
    //SI la requête de sélection retourne 1 résultat, cela veut dire que l'email ou le pseudo saisie par l'internaute est existant en BDD, alors on entre dans la condtion IF
    if($data->rowCount())
    {
        ...
        if(password_verify($_POST['password'], $user['mdp']))
        {
            ...
            //On passe en revue toute les données de l'internaute,
            // recupérées en BDD de l'internaute qui a correctement remplit le formulaire de connexion
            //qui s'est correctement connecté
        ...
        {
            $error = "<p class=' col-md-4 mx auto bg-danger text-red p-3'>Identifiant ou mot de passe erronés.</p>";
        }
    }
...
<?php if(isset($error)) echo $error; ?> <!--affichage du message d'erreur d'identifiant-->
